...
#------------------------------------------------------------------------------------------------------------------------------
    ...
    def updateFile(self):
        ...
        for local,remote in zip(self.local,self.remote):
            call('scp ' + remote + 'polength.dat ' + local, shell=True)
...
#------------------------------------------------------------------------------------------------------------------------------
    ...
    def createVectors(self):
        ...
        for path in self.local:
            self.time.append(np.genfromtxt( path + 'polength.dat', usecols=(0,) ))
            self.length.append(np.genfromtxt( path + 'polength.dat', usecols=(1,) ))
...
#------------------------------------------------------------------------------------------------------------------------------
    ...
    def plotVector(self):
        ...
        for i in range(0,4):
            axs[i].plot(self.time[i],self.length[i]         ,label=self.name[i],color='C3')   # ,linestyle=(0, (1, 1)))
            axs[i].plot(self.time[i+4],self.length[i+4]     ,label=self.name[i+4],color='C2')
            axs[i].plot(self.time[i+2*4],self.length[i+2*4] ,label=self.name[i+2*4],color='C1')
            axs[i].plot(self.time[i+3*4],self.length[i+3*4] ,label=self.name[i+3*4],color='C0') #,linestyle=(0, (1, 1)))
            axs[i].set(xlabel = r'$t/\tau_{LET} $')
            axs[i].set(ylabel = 'l/l0')
            axs[i].set_title('m=3e7 - Polymer length', y=1.05)
            axs[i].set_xlim([0,400])
            axs[i].legend()
        plt.savefig('timeVSlength.pdf')
        if self.show:
           plt.show()
